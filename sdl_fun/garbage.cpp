...
static void garbageClear(Board* board, std::map <int, Garbage*> cleared) {
   ...
   for (auto&& pair : cleared) {
      ...
      for (int c = col; c < garbage->width + col; c++) {  //clear the bottom layer
         ...
         tile->idGarbage = -1;
         tile->statusTime = clearTime + (board->game->timings.removeClear[0]);  //How long should we wait before we remove the cleared garbage
         tile->status = status_clear;
         tile->effect = visual_countdown;
         tile->effectTime = clearTime + (board->game->timings.removeClear[0]/garbage->width /2 * c + board->game->timings.removeClear[0]);  //How long should we visually fade it out
         tile->falling = false;
      }
